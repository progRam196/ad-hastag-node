...
common.ImageUpload = function(q,element,userid)
{
    ...
    var imageName = userid+userid.split("").reverse().join("")+'.'+mime_type;
...
common.profileExists = function(profile)
{
  ...
  try{
    if (fs.existsSync(appRoot+'/public/uploads/profile/'+profile) && profile != '') {
...
common.loopAdList = function(q,adresults,sessionId)
{
  try
  {
    ...
    adList[i].showEdit = parseInt(0);

    if(sessionId == element.userid)
    {
    adList[i].showEdit = parseInt(1);
    }
